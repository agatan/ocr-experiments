...
class Loss(nn.Module):
    ...
    def forward(self, loc_preds, loc_targets, conf_preds, mask):
        ...
        mask_conf = (mask > -1).unsqueeze(2)
        conf_targets = mask.unsqueeze(2).float()
        conf_loss = self.focal_loss(conf_preds[mask_conf].view(-1), conf_targets[mask_conf].view(-1))

        loss = (loc_loss + 2 * conf_loss) / n_obj
        return loss
